...
#include "gpu_dcf_sstab.h"

namespace dcf
{

    // 从 128-bit AES block 中截取一个 bout 位宽的群元素。
    ...
    {
        // 单层 DCF 遍历：同时推进下一层 seed，并累计当前层对 v_share 的贡献。
    ...
    // DCF tree 求值主 kernel：每个线程处理一个输入，并在末层把 packed leaves 合并进去。
    ...
    // 把序列化 tree key 搬到 GPU，并启动 DCF tree 求值 kernel。
    template <typename T, int E, dcfPrologue pr, dcfEpilogue ep>
    ...
    // DCF 总入口：小 bin 走 SSTable，大 bin 走 tree eval。
    template <typename T, int E, dcfPrologue pr, dcfEpilogue ep>
    ...
    // DCF keygen 主 kernel：逐层构造 scw/vcw，并在最后一层生成 packed leaves。
    template <typename T>
    ...
    {
        // 生成大 bin DCF key，并按序列化布局依次写入 scw / leaves / vcw。
    ...
    void gpuKeyGenDCF(uint8_t **key_as_bytes, int party, int bin, int bout, int N,
                      T *d_rin, T payload, AESGlobalContext *gaes, bool leq = false)
    {
        // 小 bin 直接落到 SSTable；否则走完整的 tree keygen。
        if (bin <= 8)
        {
