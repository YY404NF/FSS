...
// 沿着 DPF 树向下一层展开一个节点，并把 correction word 融入下一层 seed。
...
// DPF 默认求值路径：顺着输入 x 的路径走完整棵树，再从叶子 seed 中读出 1 bit 输出。
...
// 这条路径复用 DPF 的树遍历，但允许通过 prologue/epilogue 拼接出更高层语义。
...
// 每个线程处理一个输入元素；真正的遍历逻辑由模板参数 t 决定。
...
{
    // CPU 侧 key blob 中保存的是序列化后的只读视图；这里先搬到 GPU 再启动 kernel。
...
// DPF 总入口：小 bin 直接查 SSTable，大 bin 走树求值。
template <typename T>
...
// 兼容旧上层接口的 DCF-on-DPF 入口，同样区分小 bin 查表和大 bin 走树。
