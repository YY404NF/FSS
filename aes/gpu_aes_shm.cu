...
// 根据 128-bit 用户 key 生成 AES-128 的全部 round keys。
...
// 表驱动 AES-128 加密；DPF/DCF 把它当成 PRG 的核心。
...
{
	// 把全局常驻表搬进当前线程块的 shared memory，后续同一块内线程复用。
...
// 以 key 为种子、以 pt 为区分标签，生成 1 个 128-bit PRG 输出块。
...
// 一次生成 2 个相邻标签对应的 PRG 输出块，减少重复的 key schedule 开销。
...
// 一次生成 4 个固定标签对应的 PRG 输出块，供 tree keygen 批量展开使用。
...
{
	// 启动时把 AES 查表常量拷到 GPU，后续所有 kernel 共享这一份全局上下文。
