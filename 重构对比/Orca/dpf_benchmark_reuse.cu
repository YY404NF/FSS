...
int main(int argc, char **argv)
{
    ...
    const int evalIters = argc == 3 ? std::atoi(argv[2]) : 10;
    if (n <= 0 || evalIters <= 0)
    {
        printUsage(argv[0]);
        return 1;
    ...
    report.impl = "orca_reuse";
    report.primitive = "dpf";
    report.bin = kBin;
    report.bout = kBout;
    report.n = static_cast<unsigned long long>(n);
    report.chunk_n = static_cast<unsigned long long>(n);
    report.eval_iters = evalIters;
    report.keygen_us = static_cast<double>(microsBetween(keygenStart, keygenEnd));
    report.setup_p0_us = static_cast<double>(microsBetween(uploadP0Start, uploadP0End));
    report.setup_p1_us = static_cast<double>(microsBetween(uploadP1Start, uploadP1End));
    report.eval_p0_us = averageMicros(evalTimesP0);
    report.eval_p1_us = averageMicros(evalTimesP1);
    report.transfer_p0_us = averageMicros(transferTimesP0);
    report.transfer_p1_us = averageMicros(transferTimesP1);
    report.total_us = static_cast<double>(microsBetween(totalStart, totalEnd));
